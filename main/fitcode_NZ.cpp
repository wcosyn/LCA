...
int ho_f (const gsl_vector * x, void *data, gsl_vector * f) { 
    ...
    for (i = 0; i < n; i++)
    {
        ...
        cout << A[i] << " " << Z[i] << " " ;
        printf ("Fitted RMS, Data RMS , a parameter, b parameter,c parameter: %g %g %g %g %g\n",Yi, y[i], a, b,c);
...
void callback (const size_t iter, void *params, const gsl_multifit_nlinear_workspace *w) {
    ...
    fprintf(stdout,"iter %2zu: a = %.4f, b = %.4f, c = %.4f, cond(J) = %8.4f, |f(x)| = %.4f\n",
    iter,
    gsl_vector_get(x,0),
    gsl_vector_get(x,1),
    gsl_vector_get(x,2),
...
int main (void){
    ...
    double s[13] = {0.0363,0.0030,0.0028,0.0120,0.0022,0.0052,0.0031,0.0019,0.0020,0.0016,0.0025,0.0038,0.0013};
    double y[13] = {1.7591,1.9661,1.6755,2.5190,2.4702,2.6991,3.0610,3.4776,3.4771,3.7377,4.6538,5.4371,5.5012};
    double weights[13];
    ...
    for (i = 0; i < n; i++){
        ...
        int M = A[i]-Z[i];
        IsoMatrixElement prenorm = IsoMatrixElement((Z[i]>1?double(Z[i])*(Z[i]-1)/(A[i]*(A[i]-1)):1),(N>1?double(M)*(M-1)/(A[i]*(A[i]-1)):1),double(M)*Z[i]/(A[i]*(A[i]-1)),double(M)*Z[i]/(A[i]*(A[i]-1)));
        norm_all[i] = new norm_iso_ob(nuc_all[i], prenorm, 45.,25.,1., true);
        norm_iso_ob::norm_ob_params nob= {-1, -1, -1, -1};
        IsoMatrixElement norm_mf = norm_all[i]->sum_me_coefs( &nob );
        IsoMatrixElement norm_corr = norm_all[i]->sum_me_corr( &nob );
        IsoMatrixElement norm= norm_mf + norm_corr;
        ...
        rms_all[i] = new rms_iso_ob( nuc_all[i], norm, 45.,25.,1.,1);
    };
    struct data d = { n, y, norm_all, rms_all};
    double x_init[3] = { 45.,25.,1.}; /* starting values */
