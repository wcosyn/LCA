...
void ob(int A,int Z,std::string name, int isospin, Nucleus *nuc){
    ...
    printf("\n[Info]: ME sum is %f\n",me_sum);
    */
    spectralFunction dob3(nuc,true,true,true,norm_all,10); // nuc, central,tensor,nucl,norm,qmax (default=7)
     ...
     */
    double mf,corr;
    dob3.write(".",name.c_str(),-1,-1,-1,-1, isospin,&mf); // outputdir, outputname, nA,lA,nB,lB,t,mean field integral,corr integral
}
...
int main(int argc,char* argv[]){
    ...
    if(!pairs.compare("all")){
        ...
        NucleusIso nuc("../data/mosh","../data/mosh",A,Z,s,s2);   // inputdir,resultdir,A,Z
