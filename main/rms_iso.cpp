...
int main(int argc,char* argv[]){
    ...
    for(int i=0;i<limit;i++){
        ...
        int N=A[i]-Z[i];
        norm_iso_ob no( &nuc, IsoMatrixElement((Z[i]>1?double(Z[i])*(Z[i]-1)/(A[i]*(A[i]-1)):1),(N>1?double(N)*(N-1)/(A[i]*(A[i]-1)):1),double(N)*Z[i]/(A[i]*(A[i]-1)),double(N)*Z[i]/(A[i]*(A[i]-1))), 45.,25., true, true, true, true );
