...
//    cout << int_k << endl;
  ...
  *intmf= integral_mf;
  *intcorr= integral;
  delete i0; delete ic; delete it; delete icc; delete itt; delete ict; delete is;
  delete ics; delete ist; delete iss;
}
...
//  double p= dop->p;
  ...
  for( int ci= 0; ci < pair->get_number_of_coeff(); ci++ )
  {
    ...
    pair->getCoeff( ci, &coefi, &normi );
      ...
      int nA= coefi->getn(); int nB= coefj->getn();
      int lA= coefi->getl(); int lB= coefj->getl();
      if( nAs > -1 && nA != nAs ) continue;
      if( nBs > -1 && nB != nBs ) continue;
      if( lAs > -1 && lA != lAs ) continue;
      if( lBs > -1 && lB != lBs ) continue;
      int TA= coefi->getT(); int TB= coefj->getT();
      int MT= coefi->getMT();
      double preifactor=1;
      ...
      int NA= coefi->getN(); int NB= coefj->getN();
      int LA= coefi->getL(); int LB= coefj->getL();
      int MLA= coefi->getML(); int MLB= coefj->getML();
      int jA= coefi->getj(); int jB= coefj->getj();
      int mjA= coefi->getmj(); int mjB= coefj->getmj();
      int S= coefi->getS();
...
double density_ob3::get_me( Paircoef* pc1, Paircoef* pc2, void* params, double val )
{
  ...
  int NA= pc1->getN(); int NB= pc2->getN();
  int LA= pc1->getL(); int LB= pc2->getL();
  int MLA= pc1->getML(); int MLB= pc2->getML();
  int jA= pc1->getj(); int jB= pc2->getj();
  int mjA= pc1->getmj(); int mjB= pc2->getmj();
  int S= pc1->getS();
  /*
   * In general, projected mf needs a higher qmax than correlated part, 
