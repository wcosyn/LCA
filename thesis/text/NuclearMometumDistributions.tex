...
\caption{Blauwe curve: $n^{[1]}(k)k^2$ voor $\nuclide[12][6]{C}, \nuclide[27][13]{Al},\nuclide[40][18]{Ar},\nuclide[56][26]{Fe}, \nuclide[109][47]{Ag}, \nuclide[208][82]{Pb} $. Gestreepte rode curve: de functie \eqref{eq:powgauss} gefit aan de resultaten volgens de methode van de kleinste kwadraten. De gefitte parameters zijn $\mu$, $\sigma$,$N_norm$ en $\alpha$. De parameter $\chi^2$ (zie \eqref{eq:chisquared} met $P_{normal}(k)$ vervangen door $P_{PG}(k)$, gegeven door \eqref{eq:powgauss}.) is de goedheid van de fit. Hier is $n^{[1]}(k)$ \eqref{eq:ob_magnitude} berekend met HO  \'{e}\'{e}ndeeltjes golffuncties \eqref{eq:HO_mom_radwave} met $A$-afhankelijke parameter $\omega$ gegeven door \eqref{eq:omega}. De normalisatie is zodanig dat $\int dk\  n^{[1]}(k)k^2 =1$.}
...
Allereerst bestuderen we de relatieve tweedeeltjes impulsdistributie die we krijgen door  \eqref{eq:2BMD_uitgewerkt} te integreren over de massacentrumimpuls $\vec{P}$ over alle directionele afhankelijkheid:
...
Ook valt op dat de spreiding rond het gemiddelde ruwweg onafhankelijk is van de beschouwde kern. We kunnen besluiten dat de relatieve tweedeetjesimpuls bij benadering normaal verdeeld is voor verschillenden kernen.
...
\end{align*}
$K_{NL}(P_{12})$ is de radi\"{e}le HO golffunctie in de impulsruimte gedefinieerd in vergelijking \eqref{eq:HO_mom_radwave}.
Resultaten zijn afgebeeld in Figuur \ref{fig:twoparticledistr_cm}. $n^{[2]}(P_{12})P_{12}^2dP_{12}$ geeft de kans om twee nucleonen te vinden waarvan het massacentrum een impuls $P$ heeft in het interval $[ P_{12}, P_{12}+ dP_{12} ]$. De afwijkingen van een normale distributie vertonen dezelfde karakteristieken als bij de relatieve impulsdistributie. Contributies van relatieve orbitaal kwantumgetal $l$ zijn weergegeven ter vergelijking met een model met correlaties. Ook hier gelden dezelfde conclusies als bij de relatieve distributie in verband met de bijdragen van de verschillende $l$. De kurtosis van de berekende distributies is steeds negatief en de absolute waarde wordt steeds groter met toenemende massa $A$ van de kern. De scheefheid neemt af met de massa. De breedte van de massacentrum impulsdistributies is onafhankelijk van de massa van de kern. De scheefheid en kurtosis zijn steeds klein dus wordt de impuls van het tweedeeltjes massacentrum relatief goed beschreven door een normale verdeling.
In Figuur \ref{fig:vergelijking} vergelijken we onze resultaten van de twee distributies, hierboven besproken, met overeenkomstige GV resultaten van M. Vanhalst \cite{maarten}. Onze resultaten stemmen overeen.
...
\end{align}
Hierbij is er overgegaan naar een assenstelsel weergegeven in Figuur \ref{fig:assen}.
\begin{figure}[h]
\centering
\includegraphics[scale=0.85]{./figuren/assenstelsel1.pdf}
\caption{We kunnen een transformatie doen naar dit co\"{o}rdinatensysteem zodat alle hoeken verdwijnen behalve de hoek tussen $\vec{k}_{12}$ en $\vec{P}_{12}$: de azumitale hoek $\varphi$ is voor beide vectoren nul en ook de poolhoek $\theta_{k_{12}}$ is nul.  }
\label{fig:assen}
\end{figure}

Resultaten worden getoond in Figuur \ref{fig:angle}. Voor alle kernen zijn deze maximaal bij $\theta_{kP} = 90^{\circ}$. Als $\vec{k}_{12}$ en $\vec{P}_{12}$ loodrecht staan op elkaar dan volgt uit hun  definities \eqref{eq:rcm_impusl} dat de groottes van de individuele impulsen van de deeltjes even groot zijn, $\abs{\vec{k}_1} = \abs{\vec{k}_2}$. $n^{[2]}(\theta_{kP})\sin{\theta_{kP}}d\theta_{kP}$ geeft de kans om twee nucleonen te vinden waarvan de relatieve impuls een hoek $\theta_{kP}$ vormt met de impuls van hun massacentrum. Daar $\sin{\theta_{kP}}$ ook piekt bij $\theta_{kP} = 90^{\circ}$ is deze distributie voor alle kernen maximaal bij een loodrechte configuratie van $\vec{k}_{12}$ en $\vec{P}_{12}$. Dus twee nucleonen bevinden zich met voorkeur in een toestand waarbij ze een even grote impuls hebben.
Als we de eigenschappen van de hoekdistributies bekijken valt op dat de scheefheid steeds nul is, met andere woorden de distributies zijn steeds symmetrisch rond $\theta_{kP} = \pi/2$. Kurtosis en variantie vari\"{e}ren weinig van kern tot kern. De eerstgenoemde is duidelijk verschillend van nul en we kunnen nogmaals besluiten dat een normale distributie geen goede fit aan de resultaten zal geven.
...
\caption{De breedte $\sigma$, de kurtosis $\kappa$ en de scheefheid $\mathcal{S}$ van relatieve tweedeeltjes impulsdistributies van Figuur \ref{fig:twoparticledistr_rel}. Numerieke waarden staan in Tabel \ref{tab:properties_rel}. Daar de kurtosis steeds negatief is tonen we zijn absolute waarde.}
...
\caption{De breedte $\sigma$, de kurtosis $\kappa$ en de scheefheid $\mathcal{S}$ van massacentrum tweedeeltjes impulsdistributies van Figuur \ref{fig:twoparticledistr_cm_prob}. Numerieke waarden staan in Tabel \ref{tab:properties_cm}. Daar de kurtosis steeds negatief is tonen we zijn absolute waarde.}
...
\caption{De breedte $\sigma$ en de kurtosis $\kappa$ van de hoekdistributies tweedeeltjes impuls van Figuur \ref{fig:angle}. Scheefheid van deze distributie $\mathcal{S}$ is nul voor alle beschouwde kernen. Daar de kurtosis steeds negatief is tonen we zijn absolute waarde.}
...
De Wignerdistributies, op deze van grondtoestand na, hebben negatieve waarden. De verwachtte kwantumeffecten zijn dus aanwezig.
...
\section{Conclusie}
In een ODM is de \'{e}\'{e}ndeeltjes impulsdistributie een som van individuele bijdragen van de bezette orbitalen in een kern. Deze distributies vertonen niet-Gaussische kenmerken zoals een zekere scheefheid en kurtosis. De gemiddelde kinetische energie van een neutron en een proton in een kern, berekend aan de hand van de \'{e}\'{e}ndeeltjes impulsdistributies, ligt onder de waarde gegeven door een Fermi-model ($\braket{T}= 21\ MeV$). Ook modellen met correlaties tussen de nucleonen geven een hogere waarde omdat correlaties een brede staart bij hoge impuls introduceren ($k \gtrsim 2 fm^{-1}$) en de gemiddelde kinetische energie afhankelijk is van het vierde moment van de impulsdistributie.
 
 
Als we naar de impuls van twee nucleonen kijken in een onafhankelijke-deeltjes model kunnen we onderscheid maken tussen relatieve impuls die de twee nucleonen hebben ten opzichte van elkaar en de impuls van hun massacentrum. Zowel de verdeling van de relatieve impuls als de impuls van hun massacentrum is niet-Gaussisch en afwijkingen van een Gaussische distributie vari\"{e}ren met het massagetal van de kern. Uit vergelijking met resultaten berekend in een model met correlaties blijkt dat vooral $l=0$ wordt be\"{i}nvloed door correlaties, die in de totale distributies voor een brede staart zorgen bij hoge impulsen. GV-berekeningen zijn hier relevant voor $k \lesssim 1.5 fm^{-1}$.
...
Een ODM is een relatief goede benadering voor een kern maar het heeft beperkingen. Correlaties blijken essenti\"{e}el om de experimenteel gemeten impulsdistributies te verklaren voor grote impuls. Er is echter reeds veel onderzoek gebeurd rond de invloed van deze correlaties, bijvoorbeeld in \cite{maarten}.
...
$W(lLL_1 l_2;\Lambda x)$ stelt de Racah $W$ coefficient voor en staat in verband met de Wigner 6j-symbolen:
\begin{equation}
