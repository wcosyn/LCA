...
def twobody(k, Q_1, Q_2, M_w, i, mt_1, mt_2):
     z = 0
     j = 0
     result = 0
     f = 0
     ...
     for n1 in xrange(0,z):
          for n2 in xrange(0,j):
               for m1 in xrange(-orbital[n1],orbital[n1] + 1):
                    for m2 in xrange(-orbital[n1],orbital[n1] + 1):
                                                                                ...
                                                                                for lam in xrange(int(math.fabs(l-L)), l+L+1):
                                                                                     for M_lam in xrange(-lam,lam+1):
                                                                                          if(2*radial[n1] + orbital[n1] + 2*radial[n2] + orbital[n2] == 2*n+l+2*N+L and 2*radial[n1] + orbital[n1] + 2*radial[n2] + orbital[n2] == 2*n_ +l+2*N+L ):
                                                                                               if(((1-(-1)*(l+S+T))**2)*(clebsch_gordan(0.5,0.5,S,ms_1-0.5,ms_2-0.5,M_S)**2)*(clebsch_gordan(0.5,0.5,T,mt_1,mt_2,M_T)**2)*(clebsch_gordan(orbital[n1],orbital[n2],lam,m1,m2,M_lam)**2)*Moshinsky(n,l,N,L,radial[n1],orbital[n1],radial[n2], orbital[n2],lam)*Moshinsky(n_,l,N,L,radial[n1],orbital[n1],radial[n2], orbital[n2],lam)*Rad_wavefunc(k, v_mom(M_w,i), n, l) != 0):
                                                                                                    result = result + ((1-(-1)*(l+S+T))**2)*(clebsch_gordan(0.5,0.5,S,ms_1-0.5,ms_2-0.5,M_S)**2)*(clebsch_gordan(0.5,0.5,T,mt_1,mt_2,M_T)**2)*(clebsch_gordan(orbital[n1],orbital[n2],lam,m1,m2,M_lam)**2)*Moshinsky(n,l,N,L,radial[n1],orbital[n1],radial[n2], orbital[n2],lam)*Moshinsky(n_,l,N,L,radial[n1],orbital[n1],radial[n2], orbital[n2],lam)*Rad_wavefunc(k, v_mom(M_w,i), n, l)*Rad_wavefunc(k, v_mom(M_w,i), n_, l)*norm(v_mom(M_w,i), n, l)*norm(v_mom(M_w,i), n_, l)
     return (0.5)*result
     
     
#twobody summation for pp, nn and pn
def distribution(k, i):
     return (1.0/(A[i]*(A[i]-1)))*(twobody(k, Z[i], Z[i],M_average, i, 0.5, 0.5) + twobody(k,N[i], N[i], M_average, i, -0.5, -0.5) + twobody(k, Z[i], N[i],M_average, i, 0.5, -0.5))
     
#intgrand for normalisation integral     
def integrand(k,i) :
     return distribution(k,i)*k**2 
...
for j in nuclides:
     ...
     i = 0
     while (i < 25):
          txt = str(0.1*i) + '\t' + str(format(distribution(0.1*i, j)))
          f.write(txt)
          f.write('\n')
          i = i+1
     nor, err = sp.integrate.quad(integrand, 0, 10, args = (j))
