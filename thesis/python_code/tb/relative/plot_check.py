...
for i in kernen:
     ...
     k_Y  = Y[0:150,0] # cut from [0:25] because a lot of noise for MF for high k values!
     ...
     MF_Y = Y[0:150,1]*MF_norm
     
     dev = np.divide(MF_X,MF_Y)
     ax.plot(k_X,dev,color='b',label=nuc[i],lw=3)
     ...
     ax.set_ylabel("n^{cm}_2/n^{cm}_2_{maarten}")
     ax.set_ylim((0.6,1.4))
