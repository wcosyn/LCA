...
\end{multline}
$N_{NL} = [ 2n! / \Gamma( N + L + 3/2) ]^{1/2}$. Expanding the generalized Laguerre polynomials gives,
\begin{multline}
\int \text{d} P_{12} \, P_{12}^{2} \Pi^{*}_{N L}(P_{12}) \Pi_{N' L'}(P_{12}) \chi_{p,nl_p}^{l_1 k}(p_1,P_{12}) \chi_{p,n'l_q'}^{l_1 k'}(p_1,P_{12}) \\
= N_{NL} N_{N'L'} \nu^{ - \frac{L+L'+3}{2}}
...
\begin{multline*}
	...
	\textcolor{red}{
	\sum_{hfi=0}^{i} \sum_{hfj=0}^{j}
	(-2 \nu)^{-hfi-hfj}	
	\left( \begin{array}{c} i \\ hfi \end{array} \right)
	\left( \begin{array}{c} j \\ hfj \end{array} \right)
	(L+hfi+\frac{3}{2})^{(i-hfi)}
	(L'+hfj+\frac{3}{2})^{(j-hfj)} } \\
	\int \text{d} P_{12} \, P_{12}^{2+L+L'+\textcolor{red}{2hfi+2hfj}}  e^{- \frac{P_{12}^{2}}{ \nu}}  \chi_{p,nl_p}^{l_1 k}(p_1,P_{12}) \chi_{p,n'l_q'}^{l_1 k'}(p_1,P_{12})
\end{multline*}
We introduced the notation $a^{(n)}$ for the rising factorial of Pochhammer function. 
