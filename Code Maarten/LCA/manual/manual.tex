...
\end{align*}
However when selecting a specific isospin projection $m_t = \pm 1/2 $ (proton or neutron) we get,
\begin{align*}
	\braket{ T M_T | \delta_{m_{\tau_1},m_t} \hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} | T' M_T'}
\end{align*}
...
\end{align*}
Where $\hat{\tau}^{\pm}$ are the raising/lowering operators. Transitioning to the operators $\hat{t} = \hat{\tau}/2$ (analogues to the spin case $\hat{S} = \hat{\sigma}/2$).
\begin{align*}
	\hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} = 4 \hat{t}_{1,0} \hat{t}_{2,0} + 2 \hat{t}_{1}^{+} \hat{t}_{2}^{-} + 2 \hat{t}_{1}^{-} \hat{t}_{2}^{+} 
\end{align*}
We now investigate $ \delta_{m_{\tau_1},m_t} \hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} \ket { T M_T}$
\begin{align*}
	\delta_{m_{\tau_1},m_t} \hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} \ket { 1, 1 } &= \delta_{1,2 m_t} \ket{ 1, 1 } \\
	\delta_{m_{\tau_1},m_t} \hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} \ket { 1, 0 } &= \frac{1}{\sqrt{2}} \ket{ \frac{1}{2}, m_{t}} \otimes  \ket{ \frac{1}{2}, -m_{t}} \\
	\delta_{m_{\tau_1},m_t} \hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} \ket { 1, -1 } &= \delta_{-1,2 m_t} \ket{ 1, -1 } \\
	\delta_{m_{\tau_1},m_t} \hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} \ket { 0, 0 } &= \text{sgn}({-m_t}) \frac{3}{\sqrt{2}} \ket{ \frac{1}{2}, m_{t}} \otimes  \ket{ \frac{1}{2}, -m_{t}} \\
	&= - 2 m_t \frac{3}{\sqrt{2}} \ket{ \frac{1}{2}, m_{t}} \otimes  \ket{ \frac{1}{2}, -m_{t}}
\end{align*}
The only non-zero matrix elements for $ \braket{ T M_T | \delta_{m_{\tau_1},m_t} \hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} | T' M_T'}$ are,
\begin{align*}
	\braket{ 1,1 | \delta_{m_{\tau_1},m_t} \hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} | 1,1} &= \delta_{1,2m_t}  \\
	\braket{ 1,-1 | \delta_{m_{\tau_1},m_t} \hat{\vec{\tau}}_{1} \cdot \hat{\vec{\tau}}_{2} | 1,-1} &= \delta_{-1,2m_t}
\end{align*}
