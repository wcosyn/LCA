...
Conservation of parity means that $l_1 + l_2 = l_A + l_A = l_B + L_B$.
...
\braket{ A | \hat{n}_{s_1,t_1}^{[1],1}(\vec{p}_1) | A'} = \sum_{s_2 t_2}  \sum_{l_p = |j-1|}^{j+1} \sum_{l_q' = |j' - 1|}^{j'+1} \mathcal{M}_{AA'}^{p q,l_p l_q'}(s_1,t_1,s_2,t_2) \\
 ...
 \int \text{d} P_{12} \, P_{12}^{2} \Pi^{*}_{N L}(P_{12}) \Pi_{N' L'}(P_{12}) \chi_{p,nl_p}^{kK\dagger}(p_1,P_{12}) \chi_{q,n'l_q'}^{k'K'}(p_1,P_{12}) \\
...
64 \sqrt{2} \pi \sum_{k m_k} \sum_{K m_K} \sum_{K' m_K'} i^{-K+K'}  \\
 \int \text{d} P_{12} \, P_{12}^{2} \Pi^{*}_{N L}(P_{12}) \Pi_{N' L'}(P_{12}) \chi_{p,nl_p}^{kK}(p_1,P_{12}) \chi_{p,n'l_q'}^{kK'}(p_1,P_{12}) \\
 \int \text{d}^{2} \Omega_{r_{12}}  Y_{l_p m_{l_p} }^{*}(\Omega_{r_{12}}) Y_{k m_k}(\Omega_{r_{12}}) Y_{K m_K}^{*}(\Omega_{r_{12}}) \\
 \int \text{d}^{2} \Omega_{r_{12}'}  Y_{ l_q' m_{l_q'} }(\Omega_{r_{12}'})  Y_{k m_k}^{*}(\Omega_{r_{12}'}) Y_{K' m_K'}(\Omega_{r_{12}'}) \\
   \int \text{d}^{2} \Omega_{P_{12}}  Y_{L M_L}^{*}(\Omega_{P_{12}})  Y_{L' M_L'}(\Omega_{P_{12}}) Y_{K m_K}(\Omega_{P_{12}}) Y_{K' m_K'}^{*}(\Omega_{P_{12}})
\end{multline*}
...
\end{multline*}
We write the CGC coefficients as Wigner-3j symbols,
\begin{align*}
...
\end{multline*}
Where we flipped the signs of the summation indices $M_J$ and $m_k$, this is equivalent with simply rearranging the terms in the sums over $M_J$ and $m_k$.
...
\braket{ A | \hat{n}_{t_1}^{[1],1}(p_1) | A'} = \sum_{s_1} \braket{ A | \hat{n}_{s_1,t_1}^{[1],1}(p_1) | A'} =  \sum_{l_p = |j-1|}^{j+1} \sum_{l_q' = |j' - 1|}^{j'+1} \text{O}^{p\dagger}(S,T,j,l,l_p) \text{O}^{q}(S,T',j',l',l_q') \\
 \sum_{t_2} (-1)^{M_T + M_T' } \threej{ \frac{1}{2}, \frac{1}{2}, T, t_1,t_2, - M_T} \threej{ \frac{1}{2}, \frac{1}{2}, T', t_1,t_2, - M_T'} \\
 (-1)^{ l_p + l_q'+ m_j + m_j'}  \hat{j} \hat{j}' \threej{ l_p,S,j,m_{l_p},m_S,-m_j} \threej{l_q',S,j',m_{l_q'},m_S,-m_j'} \\
 \frac{4 \sqrt{2}}{ \pi } \sum_{l_1 m_{l_1}} \sum_{k m_k} \sum_{k' m_k'} i^{-k+k'}  \\
 \int \text{d} P_{12} \, P_{12}^{2} \Pi^{*}_{N L}(P_{12}) \Pi_{N' L'}(P_{12}) \chi_{p,nl_p}^{l_1 k}(p_1,P_{12}) \chi_{p,n'l_q'}^{l_1 k'}(p_1,P_{12}) \\
...
\end{align*}
and compare our expression against (D.37). Parts that are not found in (D.37) are colored \textcolor{red}{red}. Parts in (D.37) not appearing here are colored \textcolor{blue}{blue} (I think Maarten intended to write \textcolor{blue}{$L,L'$} in stead of \textcolor{blue}{$L_A,L_B$}).
...
\end{align*}
A closer look at the above derivation shows that the effect of $ ( \vec{r}_{12}, \vec{r}_{12}^{\, \prime} ) \rightarrow ( - \vec{r}_{12}, - \vec{r}_{12}^{\, \prime} )]$ translates into a sign switch in $i^{k-k'} \rightarrow i^{k'-k}, i^{K'-K} \rightarrow i^{K-K'}$. Suppose we are looking at a specific term in the summation over $K,K'$. (The dependence on $i^{k-k'}$ has vanished by integration over the solid angle of the one body momentum).
\begin{align*}
	\braket{ A | \hat{n}_{t_1}^{[1]}(p_1) | A'}_{K,K'} &= \braket{ A | \hat{n}_{t_1}^{[1],1}(p_1) | A'}_{K,K'} \left[ i^{K'-K} + (-1)^{S+S'+T+T'} i^{K-K'} \right] \\
	&= \braket{ A | \hat{n}_{t_1}^{[1],1}(p_1) | A'}_{K,K'} i^{K'-K} \left[ 1 + (-1)^{S+S'+T+T'+K-K'} \right]
\end{align*}
Using the fact that $ L + S + T $ must be odd we can write $ (-1)^{S+T} $ as $ (-1)(-1)^{L}$.
\begin{align*}
	\braket{ A | \hat{n}_{t_1}^{[1]}(p_1) | A'}_{K,K'} = \braket{ A | \hat{n}_{t_1}^{[1],1}(p_1) | A'}_{K,K'} i^{K'-K} \left[ 1 + (-1)^{L+L'+K-K'} \right]
\end{align*}
The correlation operators included do not change the total spin $S$, so we necessarily have $S=S'$. Allowing us to write,
\begin{align*}
	\braket{ A | \hat{n}_{t_1}^{[1]}(p_1) | A'}_{K,K'} = \braket{ A | \hat{n}_{t_1}^{[1],1}(p_1) | A'}_{K,K'} i^{K'-K} \left[ 1 + (-1)^{T+T'+K-K'} \right]
\end{align*}
Let us take a look how this is \textbf{IN A VERY OBFUSCATED WAY} dealt with in Maartens code.
\begin{verbatim}
...
\begin{itemize}
	\item $T=T'$ (\texttt{TA==TB}) \\
		As we already have $S=S'$ and $ L + S + T = $ odd, we have that $L,L'$ (\texttt{LA,LB}) must have the same parity. This means that $L-L'$ (\texttt{LA-LB}) is always even.
        \todo[inline]{!!! Re-examine this argument, I think the $L$ should actually be the relative momentum $l$, $l + S + T = $ odd. Maybe this can be tied to $L$, to be looked into later}
		\begin{itemize}
		\item \texttt{ipower1} is odd \\
			As mentioned before this means that \texttt{ipower2} necessarily is odd as well. Because $L-L'$ (\texttt{LA-LB}) is even we have that $K'-K$ and $K-K'$ (\texttt{l-la} and \texttt{la-l}) are odd. We see that if the check
			\texttt{ (ipower1 + ipower2 \% 4 == 0 )} fails the code gives an error. We will now reason that this is in fact always mathematically ensured and does not have be checked. For the current case we have \texttt{ipower1},\texttt{ipower2} $\in [-3,-1,1,3]$. We can, without loss of generality, assume that $L-L' (\texttt{LA-LB}) = 2n \; , \; n \in [-1,0,1]$, $K'-K (\texttt{l-la}) = m \; , \; m \in [-1,1]$. Hence \texttt{ipower1 + ipower2}= $4n$, $n \in [-1,0,1]$. Trivially $4n \% 4$ is always zero. This allows us to write $\texttt{ipower2} = 4n - \texttt{ipower1}$, leading to  $i^{\texttt{ipower1}} + i^{\texttt{ipower2}} = i^{\texttt{ipower1}} + i^{4n - \texttt{ipower1}} = i^{\texttt{ipower1}} - i^{\texttt{ipower1}}= 0$. As this multiplicative factor is zero, the iteration is skipped (\texttt{continue;}).
		\item \texttt{ipower1} is even \\
			If the code continues (does not skip the current iteration in the for loop) we have not entered the previous if statement (\texttt{power1} being odd) and we can assume \texttt{power1} is even ($\in [-2,0,2]$). This means \texttt{power2} is even as well ($\in [-2,0,2]$).
		...
		\end{itemize}
	\item $ T \neq T'$, (\texttt{TA != TB}) \\
		\todo[inline]{Note that as $T \neq T'$, we have $(-1)^{T+T'} = -1$ (or equivalently $(-1)^{L+L'} = -1$). Meaning that we now should multiply with $(i^{\texttt{ipower1}} - i^{\texttt{ipower2}})$ in stead of $(i^{\texttt{ipower1}} + i^{\texttt{ipower2}})$.}
		Again using the argument that $L+S+T=$ odd, $S=S'$ and the fact that $T,T' \in [0,1]$ it is easy to see that this time $L$ and $L'$ have \textbf{opposite} parity. This means that $L-L'$ (\texttt{LA-LB}) is always odd.
		\begin{itemize}
			\item \texttt{ipower1} is odd \\
				We necessarily have that \texttt{ipower2} is odd as well. As $L-L'$ (\texttt{LA-LB}) is odd we get that $K'-K$ and $K-K'$ (\texttt{l-la} and \texttt{la-l}) are even. As we have that \texttt{ipower1},\texttt{ipower2} $\in [-3,-1,1,3]$, we can, without loss of generality, assume that $L-L' (\texttt{LA-LB}) = m \; , \; m \in [-1,1]$, $K'-K (\texttt{l-la}) = 2n \; , \; n \in [-1,0,1]$. Hence \texttt{ipower1 + ipower2}= $2m$, $m \in [-1,1]$. Hence,
				\begin{verbatim}
...
or \\
\texttt{ ifactor *= i\^{}(LA-LB+l-la) ( 1 + (-1)\^{}(TA+TB+la-l) )} \\
\textbf{WHY MAARTEN, WHHYYYYYYYYYY??????????} \\
TODO: Replace this in the code and test if I get the same results...
}
...
\end{multline}
$N_{NL} = [ 2n! / \Gamma( N + L + 3/2) ]^{1/2}$. Expanding the generalized Laguerre polynomials in momentum space (see Sec.~\ref{sec:ft_transforms}, note in particular the phase factor $(-1)^{N}$, critical to the derivation) gives,
\todo[inline]{TODO: The factor $i^{L}$ is already taken into account in expressions before. Make this more clear, point this out}
\begin{multline}
\int \text{d} P_{12} \, P_{12}^{2} \Pi^{*}_{N L}(P_{12}) \Pi_{N' L'}(P_{12}) \chi_{p,nl_p}^{l_1 k}(p_1,P_{12}) \chi_{p,n'l_q'}^{l_1 k'}(p_1,P_{12}) \\
