...
\end{multline*}
Using the plane wave expansion $e^{i \vec{p} \cdot \vec{r}} = 4 \pi \sum_{l m_l} i^{l} j_{l}(p r) Y_{l m_l}^{*}(\Omega_{p}) Y_{l m_l}(\Omega_{r})$ and the fact that the isotropic harmonic oscillator wavefunctions factorize in $\psi_{n l m_l}(\vec{r}) = R_{n l}(r) Y_{l m_l}(\Omega_{r})$, $\psi_{n l m_l}(\vec{p}) = \Pi_{N L}(p) Y_{l m_l}(\Omega_{p})$, $\psi_{n (lS) j m_j}(\vec{r}) = R_{n l}(r) \mathcal{Y}_{(lS) j m_j}(\Omega_{r})$.
\begin{multline*}
	\braket{ A | \hat{n}^{[1]}(\vec{p}_1) | A'} = \\
