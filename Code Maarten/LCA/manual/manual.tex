...
\begin{itemize}
	\item $T=T'$ (\texttt{TA==TB}) \\
		...
		\item \texttt{ipower1} is odd \\
			...
			\texttt{ (ipower1 + ipower2 \% 4 == 0 )} fails the code gives an error. We will now reason that this is in fact always mathematically ensured and does not have be checked. For the current case we have \texttt{ipower1},\texttt{ipower2} $\in [-3,-1,1,3]$. We can, without loss of generality, assume that $L-L' (\texttt{LA-LB}) = 2n \; , \; n \in [-1,0,1]$, $K'-K (\texttt{l-la}) = m \; , \; m \in [-1,1]$. Hence \texttt{ipower1 + ipower2}= $4n$, $n \in [-1,0,1]$. Trivially $4n \% 4$ is always zero.
			
			Next we have the check
			\begin{verbatim}
			 if( fabs( ipower1) != fabs(ipower2) ) continue;
			 \end{verbatim}
			 We will show that this is unnecessary as well. 
		\end{itemize}
\end{itemize}
