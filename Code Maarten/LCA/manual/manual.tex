...
\newcommand{\wthreej}[6]{ % note we cant use digits in command names, something like 3j or w3j does not compute
	\ensuremath{
	(-1)^{ #1 - #2 + #6 } \sqrt{ 2 #3 + 1} \left(
	\begin{array}{c c c} 
		#1 & #2 & #3 \\
		#4 & #5 & #6
	\end{array}
...
\begin{align*}
	\braket{ j_1 m_1 j_2 m_2 | J M } = (-1)^{j_1 - j_2 + M} \sqrt{ 2J+1} 
	\left( \begin{array}{c c c}
		j_1 & j_2 & J \\
		m_1 & m_2 & M 
	\end{array}
	\right) \\
	\wthreej{j1,2,J,m1,m2,M}
\end{align*}
\begin{multline*}
\braket{ A | \hat{n}^{[1]}(\vec{p}_1) | A'} = \mathcal{M}_{A A'}^{p,q} 64 \sqrt{2} \pi \sum_{k m_k} \sum_{k' m_k'} \sum_{K m_K} \sum_{K' m_K'} (-1)^{k'+K} i^{k+k'+K+K'} Y_{k m_k}^{*}(\Omega_{p_1}) Y_{k' m_k'}(\Omega_{p_1}) \\
 \int \text{d} P_{12} \Pi_{N L}(P_{12}) \Pi_{N' L'}(P_{12}) \chi_{p,nl}^{kK\dagger}(p_1,P_{12}) \chi_{q,n'l'}^{k'K'}(p_1,P_{12}) \\
