...
sys.argv= [ "", "plot_mom_cm_np.pdf", "C", "Al", "Fe", "Pb", 12, 27, 56, 208]
...
for plot_num in range(n_plots):
  data = np.loadtxt( "dens_com_pn.00-1.%s" % sys.argv[plot_num+2] , usecols=[0,1] )
  X = data[:,0]*0.197327
  previous = data[:,1]/0.197327/0.197327/0.197327

  total0 = np.loadtxt( "hocommom_np_-10.%s" % sys.argv[plot_num+2] , usecols=[0,1] )
  total1 = np.loadtxt( "hocommom_np_-11.%s" % sys.argv[plot_num+2] , usecols=[1] )
  X_total= total0[:,0]*np.sqrt(2)
  total= (total1+ total0[:,1])/np.sqrt(8)
  ...
  if plot_num==2:
    ...
    ax.set_xticklabels(xticklabels_bold)
  if(plot_num==0):
    pl.fill_between( X, 0, previous, color='0.7', label=r"$nl=00$" )
    p1, = pl.plot(X, previous, linestyle='-', lw=3, color='0.7', label=r"$nl=00$" )  
  else:
    pl.fill_between( X, 0, previous*5, color='0.7', label=r"$nl=00$" )
    p1, = pl.plot(X, previous*5, linestyle='-', lw=3, color='0.7', label=r"$nl=00$" )  
  p2, = pl.plot( X_total, total , ls='-', color="black", lw=3, label=r"all $nl$")
#  pl.text(0.2,150, "%s" % sys.argv[plot_num+2])

  abcd=['(a)','(b)','(c)','(d)']

  if(plot_num==0):
    legend=pl.legend( [p1,p2], [ r"$\bm{nl}\bf{=}\bm{00}$",r"$\bm{\mathrm{all}}\ \bm{nl}$"], bbox_to_anchor=[0.65, 0.99], frameon=False, loc=9,
      title=r"$\bm{\mathrm{%s}\; ^{%s}\mathrm{%s}} $" % (abcd[plot_num],sys.argv[plot_num+2+size], sys.argv[plot_num+2] ) )
  else:
    legend=pl.legend( [p1,p2], [ r"$\bm{nl}\bf{=}\bm{00\ \mathrm{[x5]}}$", r"$\bm{\mathrm{all}\ nl}$"], bbox_to_anchor=[0.62, 0.99], frameon=False, loc=9,
      title=r'$\bm{\mathrm{%s}\;  ^{%s}\mathrm{%s}}$'%(abcd[plot_num],sys.argv[plot_num+2+size], sys.argv[plot_num+2]) )
