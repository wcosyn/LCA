...
if len(sys.argv) == 2:
  ...
  all= np.loadtxt("./dens_ob4.00.110.{:s}.-1-1-1-1".format(A,A))
  X= all[:,0]
  allmf= all[:,1]
  allco= all[:,2]
  allto= all[:,3]
  p00= np.loadtxt("./dens_ob4.00.110.{:s}.0000".format(A,A))
  mf00= p00[:,1]
  co00= p00[:,2]
  to00= p00[:,3]
  ...
  newf= open("./dens_ob4.00.110.{:s}.diff".format(A), "w")
...
if len(sys.argv) >= 3:
  ...
  iall=dX*sum(X*X*(allco))
  idiff=dX*sum(X*X*(codiff))
  print "integral: ", iall, i00, iall-i00
  print "diff integral: " , idiff
