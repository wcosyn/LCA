...
def MB( x, std_dev ):
  a = std_dev * m.sqrt( m.pi/ (3*m.pi-8 ) )
  return m.sqrt(2/m.pi)* x*x/ a/a/a* np.exp( -x*x/2./a/a )

def Gauss( x, std_dev ):
  return 1./m.sqrt(2*m.pi)/std_dev* np.exp( -x*x/2/std_dev/std_dev )
...
def draw_p():
  while True:
    ...
    frac= f/ distr
    if frac > 1:
      print "ERROR", r, MB(r,std_dev), distr
      continue
...
z= []
p_list= []
data= np.loadtxt( sys.argv[1], usecols= [0,2] )
X= data[:,0]
X=X
Y= data[:,1]
Y=Y
