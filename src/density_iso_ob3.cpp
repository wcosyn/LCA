...
density_iso_ob3::~density_iso_ob3()
{
...
//    cout << int_k << endl;
        ...
        IsoMatrixElement corr_t=  it.get( cf0, cft ) + itt.get( cft, cft );
        IsoMatrixElement corr_s= is.get( cf0, cfs )  + iss.get( cfs, cfs );
        IsoMatrixElement corr_ct= ( ict.get( cfc, cft ));
        ...
        {   
            for(int i=0;i<7;i++){
                ...
                *(files[i]) << endl;
                ...
                kinenergy_mf_vec[i] += kstep*k*k*k*k*(mf.getValue(i)); //does not include mass denominator!!!
                kinenergy_co_vec[i] += kstep*k*k*k*k*(corr.getValue(i)); //does not include mass denominator!!!
            }
        }
        if ( !(int_k%10) ) {
...
double density_iso_ob3::get_me( const IsoPaircoef& pc1, const IsoPaircoef& pc2, void* params, const Isolinkstrength& links)
{
    ...
    int nBs= dop->nB;
    int lBs= dop->lB;
    density_iso_ob_integrand3* integrand = dop->i0;
        ...
        for( int l = fabs( LA-q); l <= LA+q; l++ ) { //l->k' in master formula (Eq. 56 LCA manual), restriction from 3j-symbol
            for( int la= fabs( LB-q); la <= LB+q; la++ ) { //la->k in master formula (Eq. 56 LCA manual), restriction from 3j-symbol
                ...
                for( int k= max( fabs(kB-l), fabs(kA-la)) ; k <= min( kB+l, kA+la ); k++ ) {
                    ...
                    {
                        integrand->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k, val*(sum*ifactor) );
                    }
                } //k
            } //la
        } //l
...
double density_iso_ob3::get_me_corr_right( const IsoPaircoef& pc1, const IsoPaircoef& pc2, void* params, const Isolinkstrength& links)
{
    struct dens_ob_params* dop = (struct dens_ob_params*) params;
    int nAs= dop->nA;
    int lAs= dop->lA;
    ...
    density_iso_ob_integrand3* integrand_c = dop->ic;
    density_iso_ob_integrand3* integrand_t = dop->it;
    density_iso_ob_integrand3* integrand_s = dop->is;
                    ...
                    for( int k= max( fabs(kB-l), fabs(kA-la)) ; k <= min( kB+l, kA+la ); k++ ) {
                        ...
                        #pragma omp critical(add)
                        {
                            if( bcentral && mec1_check ) {
                                integrand_c->add( nB, lB, NB, LB, nA, lA, NA, LA, la, l, k, val*(mec1*sum*ifactor));
                            }
                            if( tensor && met1_check ) {
                                integrand_t->add( nB, lB, NB, LB, nA, lA, NA, LA, la, l, k, val*(met1*sum*ifactor));
                            }
                            if( spinisospin && mes1_check ) {
                                integrand_s->add( nB, lB, NB, LB, nA, lA, NA, LA, la, l, k, val*(mes1*sum*ifactor));
                            }
                        }
...
double density_iso_ob3::get_me_corr_left( const IsoPaircoef& pc1, const IsoPaircoef& pc2, void* params, const Isolinkstrength& links)
{
    struct dens_ob_params* dop = (struct dens_ob_params*) params;
    int nAs= dop->nA;
    int lAs= dop->lA;
            ...
            for( int la= fabs( LB-q); la <= LB+q; la++ ) { //la->k in master formula (Eq. 56 LCA manual), restriction from 3j-symbol
                ...
                for( int kA= jA-1; kA <= jA+1; kA++ ) {
                    ...
                    for( int k= max( fabs(kB-l), fabs(kA-la)) ; k <= min( kB+l, kA+la ); k++ ) {
                        ...
                        {
                            if( bcentral && mec1_check ) {
                                integrand_c->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k, val*(mec1*sum*ifactor*factor_right) );
                            }
                            if( tensor && met1_check ) {
                                integrand_t->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k, val*(met1*sum*ifactor*factor_right) );
                            }
                            if( spinisospin && mes1_check ) {
                                integrand_s->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k, val*(mes1*sum*ifactor*factor_right) );
                            }
                        }
...
double density_iso_ob3::get_me_corr_both( const IsoPaircoef& pc1, const IsoPaircoef& pc2, void* params, const Isolinkstrength& links )
{
    struct dens_ob_params* dop = (struct dens_ob_params*) params;
    int nAs= dop->nA;
    int lAs= dop->lA;
    ...
    density_iso_ob_integrand3* integrand_cc = dop->icc;
    density_iso_ob_integrand3* integrand_tt = dop->itt;
    density_iso_ob_integrand3* integrand_ss = dop->iss;
    density_iso_ob_integrand3* integrand_ct = dop->ict;
    density_iso_ob_integrand3* integrand_cs = dop->ics;
    density_iso_ob_integrand3* integrand_st = dop->ist;
                        ...
                        for( int k= max( fabs(kB-l), fabs(kA-la)) ; k <= min( kB+l, kA+la ); k++ ) {
                            ...
                            #pragma omp critical(add)
                            {
                                if( bcentral && mec1_check && mec2_check) {
                                    integrand_cc->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k,  val*(mec1*mec2*sum*ifactor) );
                                }
                                if( tensor && met1_check && met2_check) {
                                    integrand_tt->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k, val*(met1*met2*sum*ifactor) );
                                }
                                if( spinisospin && mes1_check && mes2_check) {
                                    integrand_ss->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k, val*(mes1*mes2*sum*ifactor) );
                                }
                                if( tensor && bcentral) {
                                    if( mec1_check && met2_check )
                                        integrand_ct->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k, val*(mec1*met2*sum*ifactor) );
                                    if( met1_check && mec2_check )
                                        integrand_ct->add( nB, lB, NB, LB, nA, lA, NA, LA, la, l, k , val*(met1*mec2*sum*ifactor) );
                                }
                                if( spinisospin && bcentral ) {
                                    if( mec1_check && mes2_check )
                                        integrand_cs->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k, val*(mec1*mes2*sum*ifactor) );
                                    if( mes1_check && mec2_check )
                                        integrand_cs->add( nB, lB, NB, LB, nA, lA, NA, LA, la, l, k ,val*(mes1*mec2*sum*ifactor) );
                                }
                                if( tensor && spinisospin) {
                                    if( mes1_check && met2_check )
                                        integrand_st->add( nA, lA, NA, LA, nB, lB, NB, LB, l, la, k, val*(mes1*met2*sum*ifactor) );
                                    if( met1_check && mes2_check )
                                        integrand_st->add( nB, lB, NB, LB, nA, lA, NA, LA, la, l, k ,val*(met1*mes2*sum*ifactor) );
                                }
                            }
                        }//k
                    }//kB
